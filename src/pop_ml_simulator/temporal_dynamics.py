...
This module provides classes and functions for modeling time-varying patient risks
using autoregressive processes, seasonal effects, and external shocks.
"""
...
class TemporalRiskSimulator:
    """
    Simulates time-varying patient risks using AR(1) process for temporal modifiers.
...
class EnhancedTemporalRiskSimulator(TemporalRiskSimulator):
    ...
    def get_shock_modifier(self) -> np.ndarray:
        ...
        for shock in self.external_shocks:
            if shock['time'] <= self.time_step < shock['time'] + shock['duration']:
                shock_modifier[shock['affected_patients']] *= shock['magnitude']
