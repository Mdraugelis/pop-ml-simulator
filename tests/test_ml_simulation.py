...
class TestMLPredictionSimulator:
    ...
    def test_achieve_target_performance(self):
        """Test that simulator can achieve target performance within tolerance."""
        simulator = MLPredictionSimulator(
            target_sensitivity=0.8,
            target_ppv=0.3,
            random_seed=42
        ...
        assert sens_error <= 0.05, f"Sensitivity error {sens_error:.3f} too large"
...
class TestEvaluationFunctions:
    ...
    def test_hosmer_lemeshow_test(self):
        ...
        calibrated_preds = self.true_labels + np.random.normal(0, 0.1, self.n_patients)
        calibrated_preds = np.clip(calibrated_preds, 0, 1)
        
        hl_stat, p_value = hosmer_lemeshow_test(self.true_labels, calibrated_preds)
        ...
        # Note: with random data, this can sometimes fail, so we use a loose threshold
...
class TestClinicalDecisionSupport:
    ...
    def setup_method(self):
        ...
        self.predictions = self.base_risks + np.random.normal(0, 0.1, self.n_patients)
        self.predictions = np.clip(self.predictions, 0, 1)
