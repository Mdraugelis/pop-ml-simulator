...
class TestEnhancedTemporalRiskSimulator(unittest.TestCase):
    ...
    def test_seasonal_effects_on_population_risk(self):
        ...
        self.assertGreater(correlation, 0.2, 
                          f"Seasonal modifier and population risk should be correlated, got {correlation}")
        ...
        self.assertGreater(power_spectrum[seasonal_freq_idx], 
        ...
        self.assertAlmostEqual(avg_risk, target_risk, delta=0.05,
                              msg=f"Average risk {avg_risk:.4f} should be close to target {target_risk:.4f}")
